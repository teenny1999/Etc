...
- 🏃 **กำลังเทรนเต็มรูปแบบอยู่** (ข้อมูลทั้งหมด 1,492+63 ภาพ, 5 epochs, CPU) บน sandbox
  คลาวด์ที่เขียนโค้ดนี้ — เริ่มไว้ก่อนเพื่อไม่ให้เสียเวลา ระหว่างที่ผู้ใช้ยังไม่ได้กลับไปที่
  เครื่องที่มี GPU สคริปต์บันทึก checkpoint ทุก epoch ไว้ที่
  `dumping_sites_fasterrcnn_checkpoint.pt` (ดู `--resume` ใน `train_detector.py`
  ถ้าต้องการโหลดมาเทรนต่อ) — ถ้า sandbox ถูกเคลียร์ก่อนเทรนเสร็จ ให้ไปต่อบนเครื่องที่มี
  GPU ตาม `CONTINUE.md` แทน (จะเร็วกว่ามาก ไม่ต้องเสียดายที่เทรนไม่จบที่นี่)
